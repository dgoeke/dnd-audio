...
| Genuine overlap keeps both audible | `TestGenuineOverlap` — canonical graph at 326400 **and** `mutual_bleed_session` (unequal levels, one `ambiguous`); both applied coefficients ≥ `overlap_min_gain_db` |
...
| Short attack, longer release, no clicks or pumping | `TestSlew` — per-frame change ≤ `1/attack_frames` rising and `1/release_frames` falling over **every** frame of every track; continuity is structural; `test_the_attack_ramp_fits_inside_the_vad_pad` pins the relationship the design rests on (OQ-019) |
| Bounded gain invariant at every frame | `TestBoundedGainShare` — `Σ g = 1` to 1e-9 **and** `c_min ≤ Σ(g·c) ≤ c_max` at every frame of every fixture, including opposite clamp extremes; plus a test that feeds the runtime checker an unnormalized matrix and proves it **fails**, because a check that cannot fire is decoration |
...
| Mono; streamed; never six waveforms in RAM | `test_mix_render.py` (mono, exact length, sample-exact against a hand-computed short case) and `test_memory.py::TestBoundedMixMemory` — M2's ordered event log extended to the **envelope**: a write happens before the last envelope chunk is *produced*, and no chunk exceeds the bounded control-frame size. _A proof over the audio path alone is passed by a renderer that materializes 690 MB of gains first and only then interleaves reads and writes._ |
| Two-pass loudness toward −16 LUFS | `test_mix_run.py::TestLoudness` — decoded MP3 within `loudness_tolerance_lu` of the target, and a session configured to −20 LUFS lands there instead, so the target is read rather than baked |
| 128 kbps mono MP3, decoded and measured, bounded retries, **fails** rather than claims compliance | `test_mix_encode.py` against a scripted measurer — compliant first time; one overshoot then compliant; and an always-overshooting one that exhausts `max_retries` and fails the stage with every measurement retained. `test_mix_run.py::TestTheMp3` through real ffmpeg — exists, decodes cleanly, mono, 128 kbps, **decoded sample count** within `duration_tolerance_frames` of `duration_samples`, session id and title in the tags |
